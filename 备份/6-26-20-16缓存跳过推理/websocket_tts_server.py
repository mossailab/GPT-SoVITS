...
CACHE_FILE = "output/tts_results/cache.json"
...
def run_tts(text):
    ...
    try:
        ...
        return os.path.splitext(os.path.basename(mp3_path))[0], file_size
...
async def cachelist_handler(request):
    # 输出全部缓存表
    cache = load_cache()
    html = "<h1>文本-音频缓存列表</h1><table border=1><tr><th>文本</th><th>MP3文件</th></tr>"
    for k, v in cache.items():
        html += f"<tr><td>{k}</td><td><a href='/download/{v}'>{v}</a></td></tr>"
    html += "</table>"
        ...
        try:
            ...
            if cmd == "开始推理":
                if not param:
                    ...
                    continue
                # 优化：查缓存
                cache_key = param.strip()
                mp3_id = cache.get(cache_key)
                mp3_path = os.path.join(OUTPUT_DIR, f"{mp3_id}.mp3") if mp3_id else None
                if mp3_id and os.path.exists(mp3_path):
                    file_size = os.path.getsize(mp3_path)
                    ...
                    print(f"[Server] 缓存命中，直接返回: {download_url}")
                else:
                    # 没有缓存，做推理
                    try:
                        mp3_id, file_size = run_tts(param)
                        cache[cache_key] = mp3_id
                        save_cache(cache)
                        download_url = f"http://43.159.42.232:8766/download/{mp3_id}"
                        await websocket.send(json.dumps({
                            "指令": "推理结果",
...
async def main():
    ...
    app.add_routes([
        ...
        web.get('/cachelist', cachelist_handler)
    ])
